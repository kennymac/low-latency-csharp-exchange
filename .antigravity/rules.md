...
* **Stochastic Mutation Check (Mutation Testing):** When explicitly requested by the user, run a stochastic mutation check on a dedicated `watchfails` branch. Inject deliberate off-by-one, memory barrier, or boundary mutations into production code, verify that tests fail (mutants killed), log failures step-by-step with individual failure/restoration commits on the branch, and document findings in `WatchTestFailures.md`.
